            ...
            <div className='flex justify-between items-center'>
                ...
                <div className='w-full hidden md:flex flex-row justify-between items-center transition ease-in-out duration-300'>
                    <div className='mx-auto '>
                        {menu.map((item, index) => (
                            <a href={item.href} key={index} className={`px-4 lg:px-8 font-medium text-white dark:text-white hover:text-green-light dark:hover:text-white hover:transition-all hover:ease-in hover:duration-200 ${item.href === pathname ? 'active' : ''}`}>{item.name}</a>
                        ))}
                    </div>
                ...
                <div className='md:hidden'>
                    <button className='p-2 bg-button-green rounded-lg' onClick={setPop}>
                        {open ?
                            ...
                            :
                            (<Image src='/icon/bar.svg' alt='ic-bar' width={18} height={18} />)
                        }
                    </button>
                </div>
            </div>
            <div className={open ? 'mx-auto transition-all ease-in duration-400 md:hidden' : 'hidden'}>
                <div className='transition ease-in-out duration-300'>
                    <div className='pt-4 pb-8 flex flex-col gap-4 justify-start items-start'>
                        {menu.map((item, index) => (
                            <a href={item.href} key={index} className={`pt-2 lg:px-8 text-sm font-semibold  text-white dark:text-white hover:text-green-light dark:hover:text-white hover:transition-all hover:ease-in hover:duration-200 ${item.href === pathname ? 'active' : ''}`}>{item.name}</a>
                        ))}
                    </div>
                </div>
            </div>
